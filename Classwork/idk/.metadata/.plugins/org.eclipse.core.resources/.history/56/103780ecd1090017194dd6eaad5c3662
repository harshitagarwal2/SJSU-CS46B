...
class Board {
	...
	public ArrayList<Board> generateAllNextBoards() {
		...
		// For each possible (row, col) pair ...
		for (int row=0; row<nCellsPerSide; row++) {
			for (int col=0; col<nCellsPerSide; col++) {
				// If (row, col) isn't occupied in this board, create a new board
				// that is identical to this board except that (row, col) is
				// occupied. Add that new board to the list that will be returned.
	...
	// A main() method for testing generateAllNextBoards(). Uses toString().
	// Note how the effort spent in writing toString() pays off in increased
	// confidence.
